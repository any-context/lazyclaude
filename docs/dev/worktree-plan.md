...
`w` キーで worktree 作成ダイアログを表示。2つの入力ダイアログ (ブランチ名 + プロンプト) を上下に同時表示し、Tab で切り替え。Ctrl+D で確定すると `.claude/worktrees/<name>/` を作成し、Claude Code を初期プロンプト付きで起動。
...
3. Tab でダイアログ間を切り替え
4. **Ctrl+D** で確定 (Enter はプロンプト内で改行入力に使用):
...
│                                       │
╰───────────────────────────────────────╯
  Ctrl+D: create  Tab: switch  Esc: cancel
```
...
- **Ctrl+D**: 両方の view から content を読み取り確定
- **Esc**: 両方削除してキャンセル
...
| `keybindings.go` | `w` rune 登録、`worktree-branch`/`worktree-prompt` view bindings (Ctrl+D/Tab/Esc) |
