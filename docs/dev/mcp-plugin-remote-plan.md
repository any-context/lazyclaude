...
対象関数:
1. `renderPluginPanel` (L13) — タブ切替後に `pluginState.remoteDisabled` を確認、true なら placeholder を出して `renderInstalledList` / `renderAvailableList` を呼ばずに return
2. `renderPluginPreview` (L91) — 同様に `remoteDisabled` 確認、true なら preview 側も placeholder
...
**注**: `renderPluginPanel` 内の `case keymap.PluginTabMCP:` は `a.renderMCPList(...)` を呼び、MCP タブ時も同じ pluginState を使う。MCP タブで `pluginState.remoteDisabled` を確認するか、`renderMCPList` 側で `mcpState.remoteDisabled` を確認するか、2 箇所 guard が必要。シンプルにするなら両方の state で確認し、どちらかが true なら placeholder。
...
// 同様に mockMCPProvider も新設 (SetProjectDir/Refresh/Servers/ToggleDenied をカウント)
```
...
| `internal/gui/render_plugins.go` | `renderPluginPanel` / `renderPluginPreview` / `renderMCPList` (MCP タブ時) に placeholder guard、共通 helper `renderRemoteDisabledPlaceholder` 追加 |
| `internal/gui/render_mcp.go` | `renderMCPList` / `renderMCPPreview` に placeholder guard |
