...
**重要 (codex v2 指摘)**: `settings.local.json` には `permissions.deniedMcpjsonServers` 以外にも他の key (他の permissions、tools 設定など) がある可能性がある。単純に `buildDeniedJSON` で新しい JSON を作って書き戻すと **remote host のユーザー設定を clobber (上書き消失)** してしまう。

Local code の `WriteDeniedServers` (`internal/mcp/config.go:70-111`) は既存 JSON を load → deniedMcpjsonServers フィールドのみ update → serialize back という **merge** 方式を取っている。Remote branch もこれに合わせる:

```go
// Remote ToggleDenied (conceptual)
existingJSON, err := m.sshReadFile(ctx, remoteSettingsLocalPath)
if err != nil {
    return fmt.Errorf("read settings: %w", err)
}
// existingJSON may be empty if file does not exist.
updatedJSON, err := updateDeniedInJSON(existingJSON, updatedDeniedList)
if err != nil {
    return fmt.Errorf("update denied in json: %w", err)
}
if err := m.sshWriteFile(ctx, remoteSettingsLocalPath, updatedJSON); err != nil {
    return err
}
```

`updateDeniedInJSON` は以下の仕様で新規 helper (`internal/mcp/config.go` に追加):
- 既存 JSON を `map[string]any` に parse
- 空文字列入力時は `map[string]any{"permissions": ...}` の最小構造を作る
- `permissions` key が無ければ作る、既存なら merge
- `permissions.deniedMcpjsonServers` を新リストで置換
- 他の keys (例: `permissions.allow`, `hooks`, `model`) は完全に保持
- Serialize back (indent 2、trailing newline)

この helper は local `WriteDeniedServers` の内部ロジックとしても再利用可能にする (refactor で local/SSH 両方から呼ぶ)。
...
3. ~~`remoteProjectPath` が `'`, `\n`, `$`, `\`` を含む場合 reject / escape?~~ → **解決済**: worker は reject (conservative) で進め、必要なら Phase 2.1 で escape に拡張
